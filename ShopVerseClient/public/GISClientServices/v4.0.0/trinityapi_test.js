		...
		if (appConfigInfo.mapDimension == "2D") {
			if (appConfigInfo.mapLib == 'ol7') {
				...
				try {
					...
					var innerLayers = [];
					var innerLayers = mapObj._layers;
					mapObj.remove(innerLayers[0]);
					console.log("LAyers~~~~~~~~~~~~", innerLayers);

					console.log("Map removed is true!!", true);
				}
				catch (e) { console.log("error", e); }
			}
		}
...
tmpl.Map.pointWithinBoundary = function (param) {
	...
	var bottomLeft = ([parseFloat(appConfigInfo.extent3), parseFloat(appConfigInfo.extent4)]);
	var topRight = ([parseFloat(appConfigInfo.extent1), parseFloat(appConfigInfo.extent2)]);
	var extent = new ol.extent.boundingExtent([bottomLeft, topRight]);

	var latlonwithin = ol.extent.containsXY(extent, parseFloat(coord[0]), parseFloat(coord[1]))

	console.log(latlonwithin)
	if (latlonwithin == true) {
		callbackFunc(true);
		console.log("true");
	} else {
		callbackFunc(false);
