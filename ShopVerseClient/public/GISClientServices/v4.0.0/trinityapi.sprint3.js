...
tmpl.Map.mapCreation = function (param) {
	...
	if (appConfigInfo.type == "none") {
		return response = { status: false,businesserrorcode: 'TB_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001',message: ' Map type is not Defined'};
	} else {
		...
		if (targetDiv == undefined || targetDiv==null || targetDiv=='' ) {
			...
			return response = { status: false,businesserrorcode: 'TB_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001',message: 'Mad Div is not Defind..'};
		}
				...
				if (appConfigInfo.mapLib == 'ol7') {
					...
					if (appConfigInfo.mapData === "google") {
						...
						if (appConfigInfo.type == "google") {
							...
							activate(map);
							if (callBackFun) {
								var response = { status: true, message: 'Map Created Successfully..', map:map };
							...
							return response = { status: true, message: 'Map Created Successfully..', map:map };
							callBackFun(response);
...
tmpl.Map.getBaseMaps = function(){
	...
	var stamen2 = new ol.layer.Tile({
		source: new ol.source.Stamen({
			...
			var hereMapSatlite =  new ol.layer.Tile({visible : true, source:new ol.source.XYZ({
			url: "https://{1-4}.aerial.maps.ls.hereapi.com/maptile/2.1/maptile/newest/hybrid.day/{z}/{x}/{y}/256/png8?apiKey=" + appConfigInfo.hereMapsAppKey
						...
						var hereMapDark =  new ol.layer.Tile({visible : true, source:new ol.source.XYZ({
			url: "https://{1-4}.base.maps.ls.hereapi.com/maptile/2.1/maptile/newest/normal.night.grey/{z}/{x}/{y}/256/png8?apiKey=" + appConfigInfo.hereMapsAppKey
							...
							var hereMapnormal =  new ol.layer.Tile({visible : true, source:new ol.source.XYZ({
...
tmpl.Map.switchBaseMaps = function (param) {
	...
	try {
		if (mapobj === null || mapobj === undefined) {
			...
			return response ={status : true, businesserrorcode: 'TB_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid Map Object'};
		}

		if (id > 5) {
			//console.log("Error message : Invalid id object parameter");
			return response ={status : true, businesserrorcode: 'TB_GISSDK_0x012', developererrorcode: 'TD_GISSDK_0x012', message: 'Invalid map ID'};
		}
	...
	if (appConfigInfo.mapDimension == '2D') {
		if (appConfigInfo.mapLib == 'ol7') {
			try {
				...
				if (id == 1) {
					...
					layers.insertAt(0, googleLayer);
						return response ={status : true, message: 'Switch to Google street Map'};
				}
				else if (id == 3) {
					...
					return response ={status : true, message: 'Switch to Google Satellite Map'};
				}
				else if (id == 4) {
		...
		try {
			...
			return response ={status : true, message: 'Switch Base Map API Enabled..'};
			...
			if (appConfigInfo.mapDimension == "2D") {
				...
				if(appConfigInfo.mapLib=="leaflet"){
					var wmsLayer = L.tileLayer.wms(layerUrl, { 
					   ...
					   transparent:true, 
					 version: '1.1.1', 
					attribution: 'WMS Layer Attribution' }).addTo(mapObj);
				}
			}
		}
		catch (err) {
...
tmpl.Overlay.create = function(param){
    ...
    for (var i = 0, length = getdata.length; i < length; i++){
		...
		// 		// rotation: anagle,									
		// 		// rotation: 0				
		// 		rotateWithView: false, // Disable rotation with view
...
if(properties != undefined){
   ...
   // Set the position of the overlay to a specific latitude and longitude
	...
	overlaymouseOver_label.setPosition(position);
		...
		//console.log("-------------",itsparent.feature.getProperties().id , popup.id );
		try
		{
		if (popup.id && itsparent.feature.getProperties().id)
		{
			...
			{
				//console.log("--",itsparent.feature.getProperties().lon,itsparent.feature.getProperties().lat);
				var tPosition = ol.proj.fromLonLat([itsparent.feature.getProperties().lon, itsparent.feature.getProperties().lat]);
				popup.setPosition(position);
				
			}
		}
		}catch(e){console.log("Fleet Dynamic Tooltip Move Object Issue..!");}
...
return response = { status: true, message: 'Enabled Route API..' };
}
...
tmpl.Zoom.toXY = function(param){
	...
	//else{
		...
		mapObj.getView().setZoom(16);
		//zoomToSearch(mapObj,lng,lat,null,null,null,null,null); //@SH added
		}else{
				...
				console.log("feature:",feature);
                    if(feature.get('id') == id)
                    {
					  ...
					  console.log("Zoomm:",extent, mapObj.getSize());
                        mapObj.getView().fit(extent, mapObj.getSize());
						 try{
						mapObj.getView().setZoom(zoomLevel);}catch(err){ console.error("error in zoomLevel param",err);}
                    }
                });
            }
        }
...
tmpl.Zoom.toXYcustomZoom = function(param){
	...
	try {
		...
		if (lat === null || lat === undefined || lat === '' || lat === "" || isNaN(lat)) {
			
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Not a Valid lat' };
		}
		

		if (lng === null || lng === undefined || lng === '' || lng === ""|| isNaN(lng)) {
			
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Not a Valid lng' };
		}
	...
	if(appConfigInfo.mapData == "google" || appConfigInfo.mapData=='hereMaps'){
		mapObj.getView().setZoom(zoomLevel);
		mapObj.getView().setCenter(ol.proj.transform([lng,lat], 'EPSG:4326', 'EPSG:3857'));
		zoomToSearch(mapObj,lng,lat,zoomLevel,null,null,null,null); //@SH added for zoom to xy
		setTimeout(function(){
			if(param.callbackFunc){
				callbackFunc();
			}
	...
	return {status : true, message : '.tmpl.Zoom.toXYcustomZoom executed successfully'}	
...
var warningIcons = document.querySelectorAll('.warning-icon');
    ...
    var iconId = clickedIcon.getAttribute('data-extra-property'); console.log('Icon clicked!',iconId);
	console.log('@@@@@$$$$$$$',iconId);
	callBack(iconId);
...
var vaultData = [];
tmpl.Map.fetchVaultData = function (param) {
	var settings = {
		"url": "http://192.168.1.87:8200/v1/secret/Dev/Platform/Gis_config/gis_Config.json",
		"method": "GET",
		"timeout": 0,
		"headers": {
		  "Content-Type": "application/json",
		  "X-Vault-Token": "7335292d-02fd-7fb9-e6bd-e8937c527ec0",
		  "Authorization": "Bearer 7335292d-02fd-7fb9-e6bd-e8937c527ec0"
		},
	  };
	  
	  $.ajax(settings).done(function (response) {
		console.log(response);
		console.log(response.data.GISClientServices.apiKey);
		localStorage.setItem('apiKey', response.data.GISClientServices.apiKey);
	  });
}
...
tmpl.Geocode.getGeocode = function (params) {
	...
	try {
		if (point === null || point === undefined || point.length == 0) {
			//console.log('tmpl.Geocode.getGeocode : Not a Valid Map Object.');
		...
		if (!callbackFunc) {
			console.log("Error message : Invalid id object parameter");
			return response = { status: false, businesserrorcode: 'TB_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid callbackFunc parameter' };
		}

	} catch (error) {
		if (error instanceof Error) {
			console.log("tmpl.Geocode.getGeocode | ", error.message);
		}
	}
...
tmpl.Geocode.getReverseGeocode = function (params) {
	...
	try {
		if (address === null || address === undefined  || address === '') {
			//console.log('tmpl.Geocode.getReverseGeocode : Not a Valid Map Object.');
		...
		if (error instanceof Error) {
			//console.log("tmpl.Geocode.getReverseGeocode : ", error.message);
			return response = { status: false, businesserrorcode: 'TB_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid parameter :' + error.message };
		}
	}
	try {
...
tmpl.Geocode.getLocality = function (params) {
	...
	try {
		if (point === null || point === undefined || point.length == 0) {
			console.log('tmpl.Geocode.getLocality : Not a Valid point Object.');
			return response = { status: false, businesserrorcode: 'TB_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid point parameter' };
		}
		else {
			console.log('tmpl.Geocode.getLocality : A Valid point parameter');
		}
		...
		if (!callbackFunc) {
			...
			console.log('tmpl.Geocode.getLocality : Valid callbackFunc param.');
		}
	} catch (error) {
		if (error instanceof Error) {
...
tmpl.Route.getRoute = function (param) {
	...
	try {
		if ( mapObj === null || mapObj === undefined || mapObj === '') {
			...
			return response = { status: false, businesserrorcode: 'TB_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid mapObj parameter' };
		}

		if (param.source === null || param.source === undefined || param.source.length == 0) {
			return response = { status: false, businesserrorcode: 'TB_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid source parameter' };
		}

		if (param.destination === null || param.destination === undefined || param.destination.length == 0) {
			return response = { status: false, businesserrorcode: 'TB_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid destination parameter' };
		}
		...
		if ((param.destinationIcon && param.sourceIcon && param.waypointsIcon) === null || (param.destinationIcon && param.sourceIcon && param.waypointsIcon) === undefined) {
			//console.log('tmpl.Route.getRoute : Not a Valid point Object.');
	...
	if (appConfigInfo.mapData === "google") {
		...
		console.log("Be patience, Will release new version soon!!");
	}
}
}
...
function CreateLayer(param, cordStart, cordEnd, coordinateArray, ETA_legs) {
		...
		var wayPointFormat = param.wayPointFormat;
		var route_width = param.route_width;
		var plotOnMap = param.plotRouteOnMap;					
		var waypoint_limit = 8;
		var wayptsExist = false;
		var format = new ol.format.WKT();
		if (param.waypoints) {
  ...
  if (plotOnMap != undefined) {
		...
		if (param.callbackFunc != undefined) {
			if (appConfigInfo.mapData == "google") {
				...
				var wktGeom = format.writeGeometry(featuresCollection_g.getGeometry().clone().transform('EPSG:3857', 'EPSG:4326'));
				 ...
				 var response = { status: true, resETA, cordStart, cordEnd, coordinateArray, ETA_legs, wktGeom,message: 'Route Created Successfully..!' };
	...
	tmpl.Measure.measure = function (param) {
		...
		try {
			...
			if (error instanceof Error) {
				console.log("tmpl.Measure.measure   | ", error.message);
			}
		}
		...
		try {
			if (appConfigInfo.mapDimension == "2D") {
				if (appConfigInfo.mapLib == "ol7") {
					...
					return {status : true, message : 'zoomtoLayer executed successfully'}
				}
	
				if (appConfigInfo.mapLib == "leaflet") {
					mapObj.fitBounds(myLayer.getBounds());
...
// ********************* SDK_MAP_API (7)Measure.area ********************* //
 tmpl.Measure.area = function (param) {
	...
	return response = { status: true, message: ' tmpl.Measure.area API Enabled..' };
}

// ********************* SDK_MAP_API (7)Measure.removeAllMarker ********************* //
...
tmpl.Map.addWMSLayer = function (param) {
	...
	try {
		...
		if (layerUrl === null || layerUrl === undefined || layerUrl === '' || layerUrl === "") {

			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid layerUrl Object' };
		}
		...
		if (layerTitle === null || layerTitle === undefined || layerTitle === '' || layerTitle === "") {
			...
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid layerTitle Object' };
		}
	...
	try {
		if (appConfigInfo.mapDimension == "2D") {
			...
			if (appConfigInfo.mapLib == "leaflet") {
				var wmsLayer = L.tileLayer.wms('http://192.168.1.211:9090/trinityMapserver/topp/wms', {
	...
	return response = { status: true, message: 'tmpl.Map.addWMSLayer API Enabled..' };
}

// ********************* SDK_MAP_API (7)removeWMSLayer********************* //
tmpl.Map.removeWMSLayer = function (param) {
	var mapObj = param.map;
	var layerTitle = param.layerTitle;
		...
		if (appConfigInfo.mapDimension == "2D") {
			...
			if (appConfigInfo.mapLib == "leaflet") {
				var wmsLayer = L.tileLayer.wms('http://192.168.1.211:9090/trinityMapserver/topp/wms', {
					...
					format: 'image/png',
					transparent: true
				}).addTo(mapObj); // To remove the WMS layer map.removeLayer(wmsLayer);

				mapObj.removeLayer(wmsLayer);
	...
	return response = { status: true, message: 'tmpl.Map.removeWMSLayer API Enabled..' };
...
tmpl.Create.point = function (param) {
   ...
   try {
	   ...
	   if (lat === null || lat === undefined || lat === '' || lat === "") {
		   ...
		   return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid lat value' };
	   }
	   

	   if (lon === null || lon === undefined || lon === '' || lon === "") {
	   
		   return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid lon value' };
	   }
   ...
   //return response = { status: true, message: 'tmpl.Create.point  API Enabled..' }; 
}
...
tmpl.Measure.clear = function (param) {
	...
	return response = { status: true, message: 'tmpl.Measure.clear  API Enabled..' };
...
tmpl.Layer.changeVisibility = function (param) {
	...
	return response = { status: true, message: 'tmpl.Layer.changeVisibility API Enabled..' };
}
       
          
// ********************* SDK_MAP_API (7)create********************* //
tmpl.Overlay.create = function (param) {
	var mapObj = param.map;
	var jsonobj = param.features;
	var layerName = param.layer;
		...
		if (appConfigInfo.mapDimension == "2D") {
			...
			if (trackLayer == false || trackLayer == undefined) {
				...
				if (OverlayisLayerPresent == false) {
					...
					overlay.setMap(mapObj);
					//mapObj.addLayer(overlay); @Shhhhhhhhh
					//if(layerSwitcher)
					//mapObj.addControl(new ol.control.LayerSwitcher());
					OverlayisLayerPresent = true;
				}
				...
				if (featureDataAry.length > 0) {
					...
					for (var j = 0; j < tmpl_setMap_layer_global_array.length; j++) {
						if (tmpl_setMap_layer_global_array[j].title == layerName) {
							OverlayisLayerPresent = true;

							tmpl_setMap_layer_global_array[j].layer.getSource().addFeatures(featureDataAry);
						}
					}
					if (OverlayisLayerPresent == false) {
						var overlay = new ol.layer.Vector({
	...
	return response = { status: true, message: 'tmpl.Overlay.create API Enabled..' };
}
...
tmpl.Overlay.addMarker = function (param) {
	...
	return response = { status: true, message: 'tmpl.Overlay.addMarker API Enabled..' };
}
       
// ********************* SDK_MAP_API (7)toFeature********************* //
tmpl.Zoom.toFeature = function (param) {
	...
	try {
		...
		if (error instanceof Error) {
			console.log("tmpl.Zoom.toFeature   | ", error.message);
		}
	}
	try {
		if (appConfigInfo.mapDimension == "2D") {
		...
		console.error("ERROR Zoom.toFeature: ", error);
	}
}
                 

///current sprint task/////////////////////////////////////////////////////////////////////////////////////////
// ********************* SDK_MAP_API mergeLine********************* //
tmpl.Route.mergeLine = function (param) {
	...
	var urlL = "http:" + appConfigInfo.connection.url + "/" + appConfigInfo.connection.project + "/apis/makeLine";
	try {
		var settings = {
			"url": urlL,
			"method": "POST",
	...
	return response = { status: true, message: 'tmpl.Route.mergeLine API Enabled..' };
}
...
tmpl.Route.buffer = function (param) {
	...
	try {
		...
		if (wktGeom === null || wktGeom === undefined || wktGeom === '' || wktGeom === "") {
			...
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid wktGeom ' };
		}
		

		if (callBackFun === null || callBackFun === undefined || callBackFun === '' || callBackFun === "") {
			
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid callBackFun' };
		}
	...
	try {
		var settings = {
			...
			"url" : "http:" + appConfigInfo.connection.url + "/" + appConfigInfo.connection.project + "/apis/getBufferPolygonArea",
			"method": "POST",
			"timeout": 0,
			"headers": {
			  "Content-Type": "application/json",
	...
	return response = { status: true, message: 'tmpl.Route.buffer API Enabled..' };
}
...
tmpl.Overlay.addGeometry = function(param){
	...
	try {
		...
		if (property === null || property === undefined || property === '' || property === "") {
			...
			return response = { status: 'false', Businesserrorcode: 'TD_GISSDK_0x001', Developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid property' };
		}
		

		if (geometryVal === null || geometryVal === undefined || geometryVal === '' || geometryVal === "") {
			
			return response = { status: 'false', Businesserrorcode: 'TD_GISSDK_0x001', Developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid geometryVal' };
		}
...
//		mapObj.addControl(new ol.control.LayerSwitcher());
	...
	return {status : true, message : 'OverlayaddGeometry  executed successfully'}
}

// ********************* SDK_MAP_API clearData********************* //
tmpl.Layer.clearData = function (param) {
	var mapObj = param.map;
	var layerName = param.layer;
	...
	return response = { status: true, message: ' tmpl.Layer.clearData API Enabled..' };
}
...
// 	try {
// 		if (mapObj === null || mapObj === undefined) {
// 			console.log('tmpl.Draw.draw : Not a Valid Map Object.');
// 			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid mapObj' };
...
tmpl.Draw.line = function (param) {
	...
	try {
		...
		if (callbackFunc === null || callbackFunc === undefined || callbackFunc === '' || callbackFunc === "") {
			...
			console.log('tmpl.Draw.line  : A Valid callbackFunc Object.');
		}
...
tmpl.Draw.clear = function (param) {
	...
	return response = { status: true, message: 'Draw.clear API Enabled' };
}
...
tmpl.Create.polygon = function (param) {
	...
	try {
		...
		if (lyrname === null || lyrname === undefined || lyrname === '' || lyrname === "") {
			...
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid lyrname Object' };
		}
		...
		if (jsonobj === null || jsonobj === undefined || jsonobj === '' || jsonobj === "") {
			...
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid jsonobj' };
		}
		


		if (strokeColor === null || strokeColor === undefined || strokeColor === '' || strokeColor === "") {
			
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid strokeColor' };
		}
	...
	return response = { status: true, message: 'Create polygon API Enabled' };
...
tmpl.Map.pointWithinBoundary = function (param) {
	...
	try { 
		...
		if (coord === null || coord === undefined || coord === '' || coord === "") {
			...
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid coord Object' };
		}
		

		if (callbackFunc === null || callbackFunc === undefined || callbackFunc === '' || callbackFunc === "") {
			
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid callbackFunc Object' };
		}
	...
	return response = { status: true, message: 'tmpl.Map.pointWithinBoundary API Enabled..' };
}
...
tmpl.Map.centerPoint = function (param) {
	...
	return response = { status: true, message: 'Map.centerPoint  API Enabled' };
}
...
tmpl.Draw.remove = function (param) {
	...
	try {
		...
		if (error instanceof Error) {
			console.log("tmpl.Draw.remove  | ", error.message);
		}
...
tmpl.Overlay.removeImage = function (param) {
	...
	return { status :true, message: "successfuly removed the overlay image"}
}
...
tmpl.Overlay.removeAllMarker = function (param) {
	...
	try {
		...
		if (error instanceof Error) {
			console.log("tmpl.Overlay.removeAllMarker  | ", error.message);return response = { status: false, businesserrorcode: 'TB_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: error.message };
...
tmpl.Overlay.markerWithName = function (param) {
	...
	try {
		...
		if (point === null || point === undefined || point === '' || point === "") {
			...
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid point Object' };
		}

			
		if (lon === null || lon === undefined || lon === '' || lon === "") {
			
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid lon Object' };
		}
		...
		if (img_url === null || img_url === undefined || img_url === '' || img_url === "") {
			...
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid img_url ' };
		}

		if (height === null || height === undefined || height === '' || height === "") {
			
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid height value' };
		}

		if ( width === null ||  width === undefined ||  width === '' ||  width === "") {
			
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid  width value' };
		}
	...
	return response = { status: true, message: 'tmpl.Overlay.markerWithName API Enabled' };
}
...
tmpl.Search.getNearestPlace = function(params){
	...
	try {
		...
		if (point === null || point === undefined || point === '' || point === "") {
			...
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid point value' };
		}
		...
		if (error instanceof Error) {
		 response = { status: false, businesserrorcode: 'TB_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: error.message };
		}
	...
	return response = { status: true, message: 'tmpl.Search.getNearestPlace API Enabled' };
}
...
tmpl.Overlay.createWithClusterWithoutCircle = function (param) {
	...
	try {
		...
		if (radius === null || radius === undefined || radius === '' || radius === "") {
			...
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid radius' };
		}

		if (distance === null || distance === undefined || distance === '' || distance === "") {

			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid distance' };
		}
		if (clusterImage === null || clusterImage === undefined || clusterImage === '' || clusterImage === "") {

			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid clusterImage' };
		}
		if (fillColor === null || fillColor === undefined || fillColor === '' || fillColor === "") {

			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid fillColor' };
		}
		if (allLayer === null || allLayer === undefined || allLayer === '' || allLayer === "") {

			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid allLayer' };
		}
		if (layerName === null || layerName === undefined || layerName === '' || layerName === "") {
		...
		if (layerSwitcher === null || layerSwitcher === undefined || layerSwitcher === '' || layerSwitcher === "") {
			...
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid layerSwitcher' };
		}
	} catch (error) {
...
tmpl.Layer.changeShape = function (param) {
	...
	try {
		...
		if (icon === null || icon === undefined || icon === '' || icon === "") {
			...
			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid icon' };
		}

		
		if (sides === null || sides === undefined || sides === '' || sides === "") {

			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid sides' };
		}

		
		if (shape === null || shape === undefined || shape === '' || shape === "") {

			return response = { status: 'false', businesserrorcode: 'TD_GISSDK_0x001', developererrorcode: 'TD_GISSDK_0x001', message: 'Invalid shape' };
		}
	...
	return response = { status: true, message: 'tmpl.Layer.changeShape API Enabled' };
}
...
tmpl.Layer.clearCircle = function (param) {
	...
	if (circleLayer != undefined)
		...
		return { status : true, message : "Successfuly cleared the circle from map"}
}
...
tmpl.Layer.clearBuffer = function (param) {
	...
	if (bufferLayer != undefined)
		...
		return response = { status: true, message: 'tmpl.Layer.clearBuffer API Enabled' };
}
...
tmpl.Map.removeFloorMap = function () {
	...
	return { status: true, message: 'tmpl.Map.removeFloorMap executed successfully' }
