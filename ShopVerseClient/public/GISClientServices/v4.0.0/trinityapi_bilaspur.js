...
gscript.src =  "https://maps.googleapis.com/maps/api/js?key="+appConfigInfo.googleMapKey+"&libraries=places,drawing,geometry&v3";
//document.getElementsByTagName("head")[0].appendChild(gscript);
console.log("Google Map API Load..");
}catch(e){
console.log("Map API Load Error",e);
}

var googleNightStyle1 = 
[
...
var googleNightStyle = [
	{"featureType":"all","elementType":"geometry","stylers":[{"color":"#242f3e"}]},
	{"featureType":"all","elementType":"labels.text.fill","stylers":[{"color":"#746855"}]},
	{"featureType":"all","elementType":"labels.text.stroke","stylers":[{"color":"#242f3e"}]},
	{"featureType":"administrative.locality","elementType":"labels.text.fill","stylers":[{"color":"#dfad86"}]},
...
tmpl.Map.createMap = function(param){
	...
	if(appConfigInfo.type == "none"){
		//return alert("API appConfigInfo.type");
		return false;
	}else{
	...
	if(appConfigInfo.mapData === "google") {
		...
		if(existingMapObject)
			...
			else
			{
				...
				var googleNightLayer = new olgm.layer.Google({  // for google night mode
					styles: googleNightStyle 
				});
				
				var darkgreyLayer = new olgm.layer.Google({  // for google night mode
					styles: darkGreyStyle 
				});
				
				var cobaltLayer = new olgm.layer.Google({  // for google night mode
					styles: cobalt 
				});
		...
		if(appConfigInfo.type == "google"){
			if(googleStyled == 'night'){ // for google night mode
				map = new ol.Map({
					...
					layers: [googleNightLayer],
					target: targetDiv,
					view: viewg,
					loadTilesWhileAnimating: true,
				});
				var layers = map.getLayers();
				layers.setAt(0, googleNightLayer); 
			}
				...
				map = new ol.Map({
					...
					layers: [darkgreyLayer],
					target: targetDiv,
					view: viewg,
					loadTilesWhileAnimating: true,
				});
				var layers = map.getLayers();
				layers.setAt(0, darkgreyLayer); 
			}
		...
		//else if(appConfigInfo.type == "sgl"){ 
			...
			layers.insertAt(0,new ol.layer.Tile({
				source: new ol.source.XYZ({
					attributions: [new ol.Attribution({
						html: 'Tiles © <a href="'+sg_url+'">ArcGIS</a>'
					})],
					url: appConfigInfo.esri_url+'/tile/{z}/{y}/{x}'
				})
			}));
		...
		if(appConfigInfo.googleSatelliteView){
			...
			streetButton.id ='ol-map-streetBtn';
			...
			//var streetBtn = document.getElementById('ol-map-streetBtn');
		...
		// map.on('pointermove', function (e) {
			...
			// map.getViewport().style.cursor = hit ? 'pointer' : 8420
		// }); 
		
		pointerMoveID = map.on('pointermove', function(event){
			enablePointerMove(map,event);
		});
		
		if(callBackFun)
			callBackFun(map);
		
		mapdivChangeAction(map);
		// tmpl.Map.resize({map: map});
...
sglLayer_Background_Map =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'Background_Map', 'TILED': true },
        serverType: 'mapserver'
    }))
})    
sglLayer_ward_boundary =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'ranchi_ward_boundary', 'TILED': true },
        serverType: 'mapserver'
    }))
})   

sglLayer_ranchi_location_feature =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'ranchi_location_feature', 'TILED': true },
        serverType: 'mapserver'
    }))
})    
sglLayer_ranchi_water_body =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'ranchi_water_body', 'TILED': true },
        serverType: 'mapserver'
    }))
})

sglLayer_ranchi_hotel =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'ranchi_hotel', 'TILED': true },
        serverType: 'mapserver'
    }))
})    
sglLayer_ranchi_thana_boundary =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'ranchi_thana_boundary', 'TILED': true },
        serverType: 'mapserver'
    }))
})

sglLayer_ranchi_parking =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'ranchi_parking', 'TILED': true },
        serverType: 'mapserver'
    }))
})    
sglLayer_ranchi_fire_station =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'ranchi_fire_station', 'TILED': true },
        serverType: 'mapserver'
    }))
})

sglLayer_ranchi_hospital =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'ranchi_hospital', 'TILED': true },
        serverType: 'mapserver'
    }))
})    
sglLayer_ranchi_streetlight =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'ranchi_streetlight', 'TILED': true },
        serverType: 'mapserver'
    }))
})

sglLayer_ranchi_property =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'ranchi_property', 'TILED': true },
        serverType: 'mapserver'
    }))
})    
sglLayer_ranchi_city_boundary =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'ranchi_city_boundary', 'TILED': true },
        serverType: 'mapserver'
    }))
})

sglLayer_ranchi_dp_boundary =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'ranchi_dp_boundary', 'TILED': true },
        serverType: 'mapserver'
    }))
})    
sglLayer_jharkhand_rails =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'jharkhand_rails', 'TILED': true },
        serverType: 'mapserver'
    }))
})

sglLayer_jharkhand_road =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'jharkhand_road', 'TILED': true },
        serverType: 'mapserver'
    }))
})    
sglLayer_jharkhand_state =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'jharkhand_state', 'TILED': true },
        serverType: 'mapserver'
    }))
})

sglLayer_jharkhand_district =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'jharkhand_district', 'TILED': true },
        serverType: 'mapserver'
    }))
})    
sglLayer_jharkhand_taluka =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'jharkhand_taluka', 'TILED': true },
        serverType: 'mapserver'
    }))
})

sglLayer_ranchi_image1 =  new ol.layer.Tile({
    source: new ol.source.TileWMS(({
        projection: 'EPSG:4326',
        url: appConfigInfo.sgl_url,//'http://115.124.113.26:1213/cgi-bin/IGiS_Ent_Service.exe?IEG_Project=PMC_WS',
        params: { 'LAYERS': 'ranchi_image1.tif', 'TILED': true },
        serverType: 'mapserver'
    }))
})

sgl_baseMap = new ol.layer.Tile({
		source: new ol.source.TileWMS(({
				projection: 'EPSG:4326',
				url: appConfigInfo.sgl_url,
				params: {
					'LAYERS': appConfigInfo.sgl_baseMap,
					'TILED': true
				},
				serverType: 'mapserver'
			}))
...
tmpl.Map.switchBaseMaps = function(param){
	...
	var layers = mapobj.getLayers();
	mapobj.removeLayer(layers.item(0));
	//mapobj.removeLayer(sglLayer);
	mapobj.removeLayer(sglLayer_ward_boundary);
	mapobj.removeLayer(sglLayer_ranchi_water_body);
	mapobj.removeLayer(sglLayer_ranchi_city_boundary);
	mapobj.removeLayer(sglLayer_ranchi_dp_boundary);
	mapobj.removeLayer(sglLayer_ranchi_thana_boundary);
	mapobj.removeLayer(sglLayer_ranchi_parking);
	mapobj.removeLayer(sglLayer_ranchi_fire_station);
	mapobj.removeLayer(sglLayer_ranchi_hospital);
	mapobj.removeLayer(sglLayer_ranchi_streetlight);
	mapobj.removeLayer(sglLayer_ranchi_property);
	mapobj.removeLayer(sglLayer_ranchi_location_feature);
	mapobj.removeLayer(sglLayer_ranchi_hotel);
	mapobj.removeLayer(sglLayer_jharkhand_rails);
	mapobj.removeLayer(sglLayer_jharkhand_road);
	mapobj.removeLayer(sglLayer_jharkhand_state);
	mapobj.removeLayer(sglLayer_jharkhand_district);
	mapobj.removeLayer(sglLayer_jharkhand_taluka);
	mapobj.removeLayer(sglLayer_ranchi_image1);
	...
	else if(id == 5)
	{
		...
		layers.insertAt(0, sgl_baseMap);
		/*
		layers.insertAt(0, sglLayer_ranchi_water_body);
		layers.insertAt(0, sglLayer_ward_boundary);
		layers.insertAt(0, sglLayer_ranchi_city_boundary);
		layers.insertAt(0, sglLayer_ranchi_dp_boundary);
		layers.insertAt(0, sglLayer_ranchi_thana_boundary);
		
		layers.insertAt(0, zone_boundary); */
	}
	else if(id == 7)
	{ // for google night mode
		...
		layers.insertAt(0, googleNightLayer);
		//mapdivChangeAction(mapobj);
		window.gMapObj7 = googleLayer;
		console.log("Map Created using New Map Object");
		}
	...
	{
		console.log("from api - HERE Map Satlite Map");
		layers.insertAt(0, baseMapLayerObjects[8]);
	}
		else if(id == 9)
	{
		console.log("from api - HERE Map Dark Map");
		layers.insertAt(0, baseMapLayerObjects[9]);
	}
			else if(id == 10)
	{
		console.log("from api - HERE Map Normal Map");
		layers.insertAt(0, baseMapLayerObjects[10]);
	}
...
tmpl.Feature.byId = function(param){
	...
	// if(existing == undefined){										// Commented by Prashanth on 01-10-2019
		// for(var i=0;i<tmpl_setMap_layer_global.length;i++){
	...
	if(existing == undefined){											// Added newly by Prashanth on 01-10-2019
		for(var i=0;i<tmpl_setMap_layer_global.length;i++){
			if(tmpl_setMap_layer_global[i].title == layrName){
				var layer = tmpl_setMap_layer_global[i].layer;
...
tmpl.Route.mergeLine = function (param) {
	...
	//	console.log(":data request:",JSON.stringify(wktGeom));

	function callback(obj) {
		if (obj) {
			obj.responseJSON.data
			var callBackFun = param.callBackFunc;
			callBackFun(obj.responseJSON.merged, obj.responseJSON.data);

		}
	}
	var urlL = appConfigInfo.connection.url + "/" + appConfigInfo.connection.project + "/apis/makeLine/";
	try {
		$.ajax({
			url: urlL,
			type: "POST",
			data: JSON.stringify({"lines": wktGeom}),
...
function getSGLRoute(param){
       ...
       routePoints.push(sourcePoint);
       //routePoints.push("["+sourcePoint[0]+","+ sourcePoint[1]+"]");
		
		for(var x=0;x<stops1.length;x++){										// Added by Prashanth by commenting function above
		var test = [parseFloat(stops1[x].lon),parseFloat(stops1[x].lat)];
		routePoints.push(test);
		//routePoints.push("["+parseFloat(stops1[x].lon)+","+parseFloat(stops1[x].lat)+"]");

		}
		
		routePoints.push(destinationPoint);
		//routePoints.push("["+destinationPoint[0]+","+ destinationPoint[1]+"]");
		
	console.log("routePoints---------------",JSON.stringify(routePoints));
	
	
	var settings = {
  "url": "https://dsclgis.uk.gov.in:1219/Home/MultipleRouteGetCombineShortestPath?srid=32643",
  "method": "POST",
  "timeout": 0,
  "headers": {
...
$.ajax(settings).done(function (response) {
	...
	for( z=0;z<res.Linestring.length;z++)
	{
		
		LineArr.push({line: res.Linestring[z]});	
		
	}
																			
		
	tmpl.Route.mergeLine ({
		lines:LineArr,
		callBackFunc:function mergedLine(mergedstatus,data){
		console.log("-----Merged Line----",mergedstatus,data);
		sglCreateRouteLayer(data,param);
	}
	});
    ...
    if (!noLayer_waypoint) {
	  ...
	  console.log("roadbuffer---->",roadbuffer);
		...
		var resETA = {};
			...
			var ETA_legs=res.response.route[0].waypoint[0].label;
			console.log("callbackFunc::",res);
			setTimeout(function(){
...
tmpl.Zoom.toLayer = function(param){
	var mapObj = param.map;
	var lyrname = param.layer;
	var callbackFunc = param.callbackFunc;			//Optional parameter added by Prashanth on 09-10-2019(request from Dency for Mangalore SWM)
	var Layers = mapObj.getLayers();
	var length = Layers.getLength();
	var existing;
	try{
			...
			if(existingLayer.get('title') == lyrname){
				...
				if(extent[0] == extent[2] && extent[1] == extent[3]){
						...
						tmpl.Zoom.toXYcustomZoom({
							map: mapObj,
							zoom: 14,
							latitude: appConfigInfo.lat,
							longitude: appConfigInfo.lon
						});
					}
...
tmpl.Layer.setIndex1 = function(param){
	var mapObj = param.map;
	var layers = param.layers;
	var lyrs = mapObj.getLayers();
	var length = lyrs.getLength();
	...
	var existing;
	var layerObjects = [];
	console.log("layers:",layers,"lyrs:",lyrs);
	for(var j=0;j<layers.length;j++){
			for(var i=0;i<tmpl_setMap_layer_global.length;i++){
				if(tmpl_setMap_layer_global[i].title == layers[j]){
					layerObjects[j] = {};
...
if(appConfigInfo.mapData=='google')
{
	...
	if(type=='nightMode')
	{
					var hereMaptrafficLayer=  new ol.layer.Tile({visible : true, source:new ol.source.XYZ({
			url: "https://{1-4}.traffic.maps.ls.hereapi.com/maptile/2.1/traffictile/newest/normal.night.grey/{z}/{x}/{y}/256/png8?apiKey=" + appConfigInfo.hereMapsAppKey
			})
					});
	hereMaptrafficLayer.setProperties({title:'hereMaptrafficLayer'});
	mapObj.addLayer(hereMaptrafficLayer);
	}
	else if(type=='dayMode')
	{
					var hereMaptrafficLayer=  new ol.layer.Tile({visible : true, source:new ol.source.XYZ({
			url: "https://{1-4}.traffic.maps.ls.hereapi.com/maptile/2.1/traffictile/newest/normal.day/{z}/{x}/{y}/256/png8?apiKey=" + appConfigInfo.hereMapsAppKey
			})
					});
	hereMaptrafficLayer.setProperties({title:'hereMaptrafficLayer'});
	mapObj.addLayer(hereMaptrafficLayer);
	}
	else if(type=='satMode')
	{
					var hereMaptrafficLayer=  new ol.layer.Tile({visible : true, source:new ol.source.XYZ({
			url: "https://{1-4}.traffic.maps.ls.hereapi.com/maptile/2.1/traffictile/newest/hybrid.day/{z}/{x}/{y}/256/png8?apiKey=" + appConfigInfo.hereMapsAppKey
			})
					});
	hereMaptrafficLayer.setProperties({title:'hereMaptrafficLayer'});
	mapObj.addLayer(hereMaptrafficLayer);
	...
	if(appConfigInfo.mapData=='google')
{
	...
	var layerTitle = 'hereMaptrafficLayer';
	var Layers = mapObj.getLayers();
	var length = Layers.getLength();
	for(var i=0;i<length;i++){
		var existingLayer=Layers.item(i);
		...
		if(this.first_add_icon == false){
			var pt = [];
					...
					if(appConfigInfo.mapData == 'google'){
						...
						pt = ol.proj.transform([pt[0],pt[1]], 'EPSG:3857', 'EPSG:4326');}
				...
				if(parent.layerFlag == false){
					var temp = newEndPt;														// Changed by Prashanth
						if(appConfigInfo.mapData == "google"){
							...
							temp = ol.proj.transform(temp, 'EPSG:4326', 'EPSG:4326');	
					}
	...
	tmpl.Draw.draw({
      ...
      callbackFunc:getDrawFeatureDetails
		...
		function getDrawFeature(coord, feature, wktGeom, value){
			...
			{
			var cordStart4326 = ol.proj.transform(cordStart, 'EPSG:4326', 'EPSG:4326');
			var cordEnd4326 = ol.proj.transform(cordEnd, 'EPSG:4326', 'EPSG:4326');
			}
			else
			{
...
//console.log("data1:",data1);
	...
	var infoCheckbox = document.createElement('input');
		...
		var haltpointCheckbox = document.createElement('input');
		haltpointCheckbox.id = "checkhaltpoint";
		haltpointCheckbox.type = "checkbox";
		haltpointCheckbox.name = "Halt Points";
		haltpointCheckbox.value = "Halt Points";
		haltpointCheckbox.checked = false;
		toggleLayersDiv.appendChild(haltpointCheckbox);	
		// haltpointCheckbox.onclick = routeLinevisibility;
		document.getElementById("toggleTrackLayers").append("Halt Points");
	...
	var tripControlsContainerStyle = document.getElementsByClassName("speedController");
		...
		tripControlsContainerStyle[0].style.width = '93px';
	...
	var optionSix = document.createElement('option');
		optionSix.value = 10;
		optionSix.innerHTML = 'x10';
		selectOption.appendChild(optionSix);
		
			var optionTen = document.createElement('option');
		optionTen.value = 20;
		optionTen.innerHTML = 'x20';
		selectOption.appendChild(optionTen);
		
		var optionSeven = document.createElement('option');
		optionSeven.value = 50;
		optionSeven.innerHTML = 'x50';
		selectOption.appendChild(optionSeven);	
		
		var optionEight = document.createElement('option');
		optionEight.value = 100;
		optionEight.innerHTML = 'x100';
		selectOption.appendChild(optionEight);
		
		var optionNine = document.createElement('option');
		optionNine.value = 300;
		optionNine.innerHTML = 'x300';
		selectOption.appendChild(optionNine);
					...
					if(lineArray.length <= 2)
					{
						...
						if(lineArray.length == 2)
						{
							...
							if(appConfigInfo.mapData == "sgl")
							{
								...
								if(prev && next)
								{
									prev = ol.proj.transform(prev, 'EPSG:4326', 'EPSG:4326');
									pres = ol.proj.transform(next, 'EPSG:4326', 'EPSG:4326');
									//alert();
									var lineString = new ol.geom.LineString([prev, pres]);
									var feature2 = new ol.Feature({
										geometry: lineString
 ...
 */
	...
	//console.log(tmpl_trip_halt_display.getSource().getFeatures().length);
		...
		if(param.haltData)
		{
		console.log("haltData--->",param.haltData);
		document.getElementById('checkhaltpoint').disabled = false;
			tmpl.Overlay.create({
			map : param.map,
			features : param.haltData,
			layer : 'haltRouteLayer',
			layerSwitcher: false
		});
		tmpl.Layer.changeVisibility({
			map : param.map,
			...
			document.getElementById("checkhaltpoint").onclick = function(){
			var checkStatus = document.getElementById("checkhaltpoint").checked;
				if(checkStatus == false){
					//alert(false);
				tmpl.Layer.changeVisibility({
						map : param.map,
		...
		console.log("Halt Point Layer haltData Missing");
		document.getElementById('checkhaltpoint').disabled = true;
		}
		}catch(e){
		console.log("Halt Point Layer Creation Error..!",e);
		}
...
function sendDataToDrawAnimationLineTemp(){
	...
	var prev = [parseFloat(uniqueData[k - 1].lon),parseFloat(uniqueData[k - 1].lat)];
		...
		}else if(appConfigInfo.mapData=='hereMaps')
		{
			...
			prev = ol.proj.transform(prev, 'EPSG:4326', 'EPSG:4326');
			pres = ol.proj.transform(pres, 'EPSG:4326', 'EPSG:4326');
		}
...
tmpl.Map.addWMSLayer = function(param){
	...
	var layerTitle = param.layerName;
...
tmpl.Create.circle = function(param){
	...
	if(strokeColor && fillColor && strokeColor!=null && fillColor!=null ){
		style = new ol.style.Style({
	        stroke: new ol.style.Stroke({
	            color: strokeColor,
	            width: 1
 	...
 	if(appConfigInfo.mapData == 'google' || appConfigInfo.mapData=='hereMaps')
    {
       ...
       cirGeomtry4326 = cirGeomtry.clone().transform('EPSG:3857', 'EPSG:4326');
       wktBufferGeom= format.writeGeometry(cirGeomtry4326);   
	   // var modify = new ol.interaction.Modify({source: new ol.source.Vector()});			//Added by Prashanth, delete it later
...
tmpl.POI.addPOICircleGeometry = function(param) {
    ...
    for (var i = 0; i < features.length; i++) {
        ...
        feature.setStyle(new ol.style.Style({
            image: new ol.style.Icon(({
				...
				size: features[i].img_size//[32,32]
            }))
        }));
        var keyNames = Object.keys(features[i]);
        for (var name = 0; name < keyNames.length; name++) {
    ...
    for (var j = 0; j < features.length; j++) {
        ...
        if (appConfigInfo.mapData == 'google' || appConfigInfo.mapData=='hereMaps') {
            ...
            cirGeomtry4326 = cirGeomtry.clone().transform('EPSG:3857', 'EPSG:4326');
            wktBufferGeom = format.writeGeometry(cirGeomtry4326);
			// var ids = features[j].id;
			// var coord = [lonc, latc];
			// var layerName = lyrName;
			// var properties1 = features[j];
			// var mapObject = mapObj;
			// getOverlayFeatureDetails(ids,coord,layerName,properties1,mapObject);
        } else {
            circle4326 = ol.geom.Polygon.circular(wgs84Sphere, [lonc, latc], features[j].radius, 64);
            circle3857 = circle4326;
            circleFeature = new ol.Feature(circle3857);
...
console.log("cRadius:",cRadius);
	var propertyId = param.id;
	var lyrName = param.layerName;
	var changeLatLog = param.changeLatLog;
	
	if(param.radius)
	{
	 cRadius = param.radius;	
	}else{
		cRadius=50;
	}
...
if(changeLatLog)
{
		...
		if(appConfigInfo.mapData == 'google' || appConfigInfo.mapData=='hereMaps')
	    {
	       circle4326 = ol.geom.Polygon.circular(wgs84Sphere, coord, cRadius, 64); // circle4326 = ol.geom.Polygon.circular(wgs84Sphere, coord, circleRadius, 64);
		   circle3857 = circle4326.clone().transform('EPSG:4326', 'EPSG:3857');
	       circleFeature =new ol.Feature(circle3857);
	       circleFeature.setStyle(style);
        ...
        function handleGeocode(data){
        	address = data.address;
        	var rec = {point:wktGeom,radius:cRadius,address:address,geometry:wktBufferGeom};
			callbackFunc(rec);
        }
...
tmpl.Overlay.addKML = function(param){
	var mapObj = param.map;
	var url = param.url;
	var layerName = param.layer;
	var id = param.id;
	...
	vector.setProperties({title: layerName, id: id});
	
	// vector.setMap(mapObj);	
	mapObj.addLayer(vector);
}


tmpl.Overlay.removeKML = function(param){
	var map = param.map;
	var id = param.id;
	var layerName = param.layer;
	
	try{
		var existingLayer = map.getLayers();
		if(existingLayer != undefined){
			var i = 0;
			while(i<existingLayer.getLength()){
				var existing = null;
				existing = existingLayer.item(i);
				if(existing.get('title') == layerName){
					if(existing.get('id') == id){
						map.removeLayer(existing);
					}
					else{
						console.error("Layer name and ID does not match");
					}				
				}
				i++;
			}
		...
		console.error("ERROR Overlay.removeKML: ",err);
	}
}


tmpl.Overlay.changeKMLvisibility = function(param){
	var map = param.map;
	var id = param.id;
	var layerName = param.layer;
	var visibility = param.visible;
	...
	try{
		...
		if(existingLayer != undefined){
			...
			while(i<existingLayer.getLength()){
				var existing = null;
				var existing = existingLayer.item(i);
				if(existing.get('title') == layerName){
					if(existing.get('id') == id){
						existing.setVisible(visibility);
					}
					else{
						console.error("Layer name and ID does not match");
					}				
...
tmpl.Route.buffer = function(param){
	...
	var format = new ol.format.WKT();
		...
		var input_turf = turf.lineString(p);
		var buffered = turf.buffer(input_turf, 0.001, {units:'kilometers'});

		
		  var wktGeomFinal = "POLYGON((";
			for (var j = 0; j < buffered.geometry.coordinates[0].length; j++) {
				if (j == buffered.geometry.coordinates[0].length - 1)
					wktGeomFinal += buffered.geometry.coordinates[0][j][0] + ' ' + buffered.geometry.coordinates[0][j][1];
				else
					wktGeomFinal += buffered.geometry.coordinates[0][j][0] + ' ' + buffered.geometry.coordinates[0][j][1] + ',';
			}
			wktGeomFinal += "))";
			 console.log("buffered wktGeom Converted::",wktGeomFinal);
			 callBackFun(wktGeomFinal);
	/*$.ajax({
			type:'POST',
			url:urlL,
			data:{
 ...
 function bindInputs(zindex, layer) {
	layer.setZIndex(parseInt(zindex, 10) || 0);
}
